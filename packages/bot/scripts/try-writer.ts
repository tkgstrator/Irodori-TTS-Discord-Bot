...
const main = async (): Promise<void> => {
  ...
  console.log(
    `GEMINI_API_KEY: ${
      config.GEMINI_API_KEY
        ? `[${config.GEMINI_API_KEY.length} chars, starts with ${config.GEMINI_API_KEY.slice(0, 10)}...]`
        : 'MISSING (.env or process env にロードされていない)'
    }`
  )
  ...
  console.log('')
  const dumpPath = `/tmp/gemini-writer-${Date.now()}.json`
  console.log(`Writer に問い合わせ中... (Gemini, raw は ${dumpPath} に保存)`)

  const startedAt = Date.now()
  const vds = await write(sheet, { dumpRawTo: dumpPath })
