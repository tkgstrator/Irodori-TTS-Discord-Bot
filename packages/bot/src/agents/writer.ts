...
import { LooseVdsJsonSchema, type VdsJson, VdsJsonSchema } from '../schemas/voice-drama.dto'
...
- \`version\`: 常に整数 \`1\`
- \`speakers\`: alias → **\`{ "uuid": "<uuid>" }\` のみ** のマップ。**uuid 以外のフィールドは絶対に含めない**。
...
\`\`\`json
"speakers": {
  "emma": { "uuid": "...", "persona": "...", "firstPerson": "boku", ... }
}
\`\`\`
...
\`\`\`json
"speakers": {
  "emma":    { "uuid": "7c9e6a55-5b6a-4a4d-9c49-1d5a3b2f6cbb" },
  "hiro":    { "uuid": "5680ac39-43c9-487a-bc3e-018c0d29cc38" },
  "narrator":{ "uuid": "00000000-0000-4000-8000-000000000000" }
}
\`\`\`
...
## cue の 2 種類（厳密、これ以外の kind は一切許されない）
- **speech cue**: \`{ "kind": "speech", "speaker": "<alias>", "text": "<セリフ>" }\`
- **pause cue**:  \`{ "kind": "pause", "duration": <秒数> }\`

**\`kind\` に許される値は文字列 "speech" と "pause" のみ**。\`"narration"\` / \`"sfx"\` /
\`"description"\` / \`"scene"\` 等の独自値は絶対に使わない。ナレーションが必要な場合は、
...
- \`boku\`→「ボク」or「僕」／\`ore\`→「俺」／\`uchi\`→「うち」
- \`washi\`→「ワシ」／\`ware\`→「我」／\`yo\`→「余」／\`soregashi\`→「それがし」
...
3. 無ければ話者の \`defaultHonorific\` を相手の \`name\` に付けて呼ぶ。enum → 敬称:
   \`none\`→呼び捨て／\`san\`→「◯◯さん」／\`chan\`→「◯◯ちゃん」／
   \`kun\`→「◯◯君（くん）」／\`sama\`→「◯◯様」／\`senpai\`→「◯◯先輩」／
   \`sensei\`→「◯◯先生」／\`tan\`→「◯◯たん」／\`dono\`→「◯◯殿」
   相手の name から姓・名のどちらを使うかは文脈に合わせる（例: 「桜羽エマ」 + \`chan\` → 自然なのは「エマちゃん」）。
...
const buildWriteSchema = (maxCueTextLength: number) =>
  LooseVdsJsonSchema.transform((raw) => splitLongSpeechCues(raw, maxCueTextLength)).pipe(
    VdsJsonSchema
  )
