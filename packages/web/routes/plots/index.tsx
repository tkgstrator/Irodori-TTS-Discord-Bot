  ...
  return (
    ...
    >
      <div className="flex flex-1 flex-col p-3.5">
        ...
        <div className="mb-3 rounded-lg border border-border/70 bg-secondary/30 px-3 py-2.5">
          <div className="flex items-start justify-between gap-3">
            <div>
              <p className="text-[0.625rem] font-medium tracking-wide text-muted-foreground">章進捗</p>
              <p className="mt-0.5 text-sm font-semibold">
                {chapterMeta.latestNumber > 0 ? `第${chapterMeta.latestNumber}章まで作成` : 'まだ章は未作成'}
              </p>
            </div>
            <div className="text-right">
              <p className="text-base font-semibold leading-none">
                {chapterMeta.completedCount}
                <span className="ml-1 text-xs text-muted-foreground">/ {chapterMeta.totalCount}</span>
              </p>
              <p className="mt-1 text-[0.625rem] text-muted-foreground">完了章数</p>
            </div>
          </div>
        ...
        <div className={`mt-auto grid grid-cols-3 gap-2 ${scenario.status === 'draft' ? 'opacity-50' : ''}`}>
          <div className="flex flex-col items-center rounded-lg bg-secondary/50 px-2 py-1.5">
            ...
            <span className="mt-1 text-sm font-semibold">{chapterMeta.totalCount}</span>
            <span className="text-[0.625rem] text-muted-foreground">全章</span>
          </div>
          <div className="flex flex-col items-center rounded-lg bg-secondary/50 px-2 py-1.5">
...
const PlotsPage = () => {
  ...
  return (
    <div className="mx-auto max-w-6xl p-4 pb-8 sm:p-6">
      <div className="mb-6">
        <h1 className="text-2xl font-bold tracking-tight">プロット管理</h1>
        <p className="mt-0.5 text-sm text-muted-foreground">どこまで章が出来ているかを一覧で確認できます</p>
      </div>
