...
const isDarkMode = (theme: string): boolean => {
  if (theme === 'dark') return true
  if (theme === 'light') return false
  return typeof window !== 'undefined' && window.matchMedia('(prefers-color-scheme: dark)').matches
}

const getEdgeColor = (type: RelationType, dark: boolean): string => {
...
const CharacterNodeComponent = ({ data }: NodeProps<CharacterNode>) => {
...
export const RelationsPageWrapper = () => {
  return (
    <ReactFlowProvider>
      <RelationsPage />
    </ReactFlowProvider>
...
const buildNode = (char: RelationCharacter, position: { x: number; y: number }, selectedId: string): CharacterNode => {
  return {
    id: char.id,
